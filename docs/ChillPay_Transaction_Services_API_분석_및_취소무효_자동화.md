...
| 구분 | 설명 | ChillPay 노티 | 처리 방식 |
|------|------|----------------|-----------|
| **취소 (Cancel)** | 결제 중간 이슈로 취소 | ✅ 노티 발송 | - |
| **무효 (Void)** | 결제 완료 후 건 취소(미정산) | ❌ 노티 미발송 | 시간대별 아래 참조 |
| **환불 (Refund)** | 이미 정산된 건 취소 | ❌ 노티 미발송 | 수동(메뉴얼), 비용 발생 |

### 1.1 취소 가능 시간 – 환경 설정 (한국·일본 시간 기준)

**기준 시간대**: 한국(KST) 및 일본(JST) 동일 적용. **환경 설정에서 취소 가능 시간을 세팅**하며, **기본값**은 아래와 같다.
